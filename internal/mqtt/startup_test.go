...
func TestStartupCommands(t *testing.T) {
	...
	assert.Equal(t, "homeassistant/device/nest_controller_1_unit/config", commands[1].Publish.Topic)
	assert.True(t, commands[1].Publish.Retain)
	assert.Contains(t, string(commands[1].Publish.Payload), `"cmps"`)
	...
	assert.True(t, commands[2].Publish.Retain)
}
